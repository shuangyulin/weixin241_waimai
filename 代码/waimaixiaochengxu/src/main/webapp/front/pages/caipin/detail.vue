		...
		<view class="container">
			...
			<view class="detail-content">
				<view class="price detail-list-item priceFavor"
					...
					<view class="text" style="color: red;textAlign:left">
						<text style='font-size:20rpx'>¥</text><text style="font-weight:bold;font-size:50rpx">{{detail.caipinNewMoney}}</text><text style="font-size: 20rpx;color: #8799A3;margin-left: 10rpx;">
							<text v-if="detail.caipinOldMoney!=null">原价￥<text style="text-decoration:line-through">{{detail.caipinOldMoney}}</text></text>
                    	</text>
					</view>
					...
					<view class="name shop-title"
						  :style='{"boxShadow":"0 0 6rpx rgba(0,0,0,.3)","margin":"0 0 20rpx 0","borderColor":"#fff","backgroundColor":"#fff","color":"#A27DE3","borderRadius":"0","borderWidth":"0","fontSize":"36rpx","borderStyle":"solid"}'>
						菜品名称：{{detail.caipinName}}
					</view>
			...
			<view class="time-content"
				...
				<view class="content" style="line-height: 50upx;letter-spacing: 5upx;"
					...
					<rich-text :nodes="detail.caipinContent"></rich-text>
				</view>
			</view>
...
<script>
	export default {
		...
		methods: {
			...
			// 获取详情
			async init() {
				let res = await this.$api.info('caipin', this.id);
				this.detail = res.data;
				// 主图
				this.swiperList = this.detail.caipinPhoto ? this.detail.caipinPhoto.split(",") : [];
			},
			//添加购物车
            async onCartTap() {
            ...
            onSubmit(){
                uni.setStorageSync('orderGoods', [{
                        caipinId: this.detail.id,
                        caipinName: this.detail.caipinName,
                        caipinPhoto: this.swiperList[0],
                    buyNumber: 1,
                        caipinNewMoney: this.detail.caipinNewMoney,
                }])
                this.$utils.jump('../caipinOrder/confirm');
            },
			// 收藏
			async getcaipinCollection() {
                let params = {
                    page: 1,
                    limit: 1,
                    caipinId: this.id,
                ...
                let res = await this.$api.list(`caipinCollection`, params);
			...
			async shoucang() {
                ...
                let res = await _this.$api.list(`caipinCollection`, params);
                if (res.data.list.length == 1) {
                    let caipinCollectionId = res.data.list[0].id;
                    uni.showModal({
                        title: '提示',
                        content: '是否取消',
                        success: async function(res) {
                            if (res.confirm) {
                                await _this.$api.del('caipinCollection', JSON.stringify([caipinCollectionId]));
                                _this.$utils.msg('取消成功');
                                _this.getcaipinCollection();
                            }
                        }
                    });
                    return;
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            await _this.$api.add('caipinCollection', {
                                yonghuId: _this.user.id,
                                caipinId: _this.detail.id,
                                caipinCollectionTypes:1
                            });
			...
			async upCallback(mescroll) {
				let res = await this.$api.list('caipinCommentback', {
					page: mescroll.num,
					limit: mescroll.size,
					caipinId: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
                this.$utils.jump(`../caipinCommentback/add-or-update?caipinId=${this.id}`)
            },
		}
	}
</script>
