<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input v-model="searchForm.caipinName" type="text" placeholder="菜品名称"></input>
		</view>
	...
	<scroll-view scroll-x="true" class="tabView"
		...
		<view :style='categoryName===item.codeIndex?sactiveItem:sitem' class="tab"
			  v-for="(item,index) in caipinTypesList" :key="index"
...
<view class="list" :style='{"borderRadius":0,"backgroundColor":"rgba(255, 255, 255, 0)"}'>
    <view style="margin: 10rpx 10rpx 10rpx 10rpx;"
            ...
            :style='{"borderRadius":"40rpx","backgroundColor":"#A27DE3"}'
        ...
        <image :style='{"borderRadius":"40rpx","width":"50%","height":"200rpx"}' mode="aspectFill" class="listmpic"
               :src="product.caipinPhoto?product.caipinPhoto.split(',')[0]:''"></image>
        <view class="listmr">
           ...
           <view style="display: flex;justify-content: space-between;float: left;margin-top: 20%;">
            <text style="margin-left: 50rpx;" v-if="isAuth('caipin','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">
                修改
            </text>
            <text v-if="isAuth('caipin','删除')" class="cuIcon-delete"
            ...
            v-if="isAuth('caipin','新增')" class="add-btn" @click="onAddTap()">新增</button>
	...
	export default {
		...
		async onShow() {
            ...
            _this.btnColor = _this.btnColor.sort(() => {
	   		...
	   		//当前表的 菜品类型 字段 字典表查询方法
		  	let caipinTypes = await this.$api.page('dictionary', {
			 	 page: 1,
			  	limit: 100,
		  		dicCode:'caipin_types'
		  	});
		  	this.caipinTypesList = caipinTypes.data.list;
		...
		methods: {
				...
				let params = {
                    ...
                    ,shangxiaTypes: 1
							...
							if (this.categoryName != 0) {
                                params.caipinTypes = this.categoryName
                            }

                let res = await this.$api.list(`caipin`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('caipin', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search() {
				...
				if (this.searchForm.caipinUuidNumber) {
                    searchForm['caipinUuidNumber'] = this.searchForm.caipinUuidNumber
                }
				if (this.searchForm.caipinName) {
                    searchForm['caipinName'] = this.searchForm.caipinName
                }
				if (this.searchForm.caipinName) {
					searchForm['caipinName'] = this.searchForm.caipinName
				}

                let res = await this.$api.list(`caipin`, searchForm);
				// 如果是第一页数据置空
